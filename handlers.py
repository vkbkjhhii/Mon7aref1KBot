...
async def generate_visa(callback_query: types.CallbackQuery):
    ...
    names = ["Paolo Bernhard", "John Carter", "Michael Smith", "Daniel Brown", "Robert Wilson"]
...
async def get_link(message: types.Message):
    ...
    url = message.text
    try:
        short = requests.get(f"https://tinyurl.com/api-create.php?url={url}").text
    except:
        await message.reply("❌ حدث خطأ في اختصار الرابط، حاول مرة أخرى")
        return

    user_links[message.from_user.id].append(short)

    # جهز الرسالة مع كل الروابط اللي المستخدم عملها
    text = "✅ روابطك المختصرة:\n\n"
    for i, link in enumerate(user_links[message.from_user.id], 1):
        text += f"{i}. {link}\n"
